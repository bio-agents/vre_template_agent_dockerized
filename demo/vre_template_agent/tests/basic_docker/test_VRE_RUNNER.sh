...
###
### Test VRE agent execution (local)
###
...
echo '# Start time:' $(date) > $WORKING_DIR_HOST/agent.log
...
cmd="docker run \
	...
	$TOOL_IMAGE \
	$TOOL_EXECUTABLE --config $WORKING_DIR_DOCKER/config.json --in_metadata $WORKING_DIR_DOCKER/in_metadata.json --out_metadata $WORKING_DIR_DOCKER/out_metadata.json --log_file $WORKING_DIR_DOCKER/agent.log
"

echo $cmd
$cmd >>  $WORKING_DIR_HOST/agent.log 2>&1
echo "--- End time: $(date)"
